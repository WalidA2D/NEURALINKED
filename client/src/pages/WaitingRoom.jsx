...
function WaitingShell() {
  ...
  const { socket, connected, code, players, hostId, startGame, leaveRoom } = useRoom();
  const me = "me"; // en vrai: ton userId
  const isHost = hostId === "me" || new URLSearchParams(location.search).get("host")==="1";
//   changer
      ...
      <main className="auth__panel">
        ...
        <section className="card" style={{ display:"grid", gap:12 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
            <div className="pill">Code : <strong style={{marginLeft:6}}>{code}</strong></div>
          ...
          <ul style={{ margin:0, paddingLeft:18 }}>
            {players.map((p)=>(
              <li key={p.id || p.name}>{p.name}{(p.id==="me") ? " (moi)" : ""}{(hostId===p.id) ? " ⭐" : ""}</li>
            ))}
          </ul>
